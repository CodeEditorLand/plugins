...
use swc_common::{FileName, comments::Comments, pass::Optional};
use swc_ecma_ast::Pass;
use swc_ecma_visit::VisitMut;

pub use crate::{
	utils::{State, analyze, analyzer},
	visitors::{
		display_name_and_id::display_name_and_id,
		minify::visitor::minify,
...
fn true_by_default() -> bool { true }

fn default_index_file_name() -> Vec<String> { vec!["index".to_string()] }
...
where
	...
	(
		...
		Optional { enabled:config.css_prop, visitor:transpile_css_prop(state.clone()) },
		Optional { enabled:config.minify, visitor:minify(state.clone()) },
		...
		Optional { enabled:config.pure, visitor:pure_annotation(comments, state) },
	)
}
