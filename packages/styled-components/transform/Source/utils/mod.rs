...
impl State {
	pub(crate) fn is_styled(&self, tag:&Expr) -> bool {
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = tag {
		...
		if let Some(style_required) = self.styled_required.clone() {
			match tag {
				Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(..), .. }) => {
	...
	pub(crate) fn set_import_name(&mut self, id:Id) { self.imported_local_name = Some(id); }
