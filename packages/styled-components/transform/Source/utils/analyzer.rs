...
impl VisitMut for AsAnalyzer {
	...
	fn visit_mut_module(&mut self, p:&mut Module) {
		let mut v = Analyzer { config:&self.config, state:&mut self.state.borrow_mut() };
...
pub fn analyze(config:&Config, program:&Program) -> State {
	...
	let mut v = Analyzer { config, state:&mut state };
...
impl Visit for Analyzer<'_> {
	...
	fn visit_var_declarator(&mut self, v:&VarDeclarator) {
		...
		if let (
			Pat::Ident(name),
			Some(Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. })),
