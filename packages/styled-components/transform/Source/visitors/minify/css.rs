...
use super::{
	css_placeholder::{PLACEHOLDER_REGEX, make_placeholder, split_by_placeholders},
...
static LINEBREAK_REGEX_RAW:Lazy<Regex> = Lazy::new(|| Regex::new(r"(?:\\r|\\n|\r|\n)\s*").unwrap());
...
fn compress_symbols(code:impl AsRef<str>) -> String {
	split_keep(&SYMBOL_REGEX, code.as_ref()).into_iter().enumerate().fold(
		"".to_string(),
		|s, (index, fragment)| {
...
fn is_line_comment(s:impl AsRef<str>) -> bool { s.as_ref().trim_start().starts_with("//") }
...
fn minify_values(
	...
	let minified_values =
		split_by_placeholders(&minified_code).into_iter().map(Atom::from).collect();
	...
	MinifyResult { values:minified_values, retained_expression_indices }
}
...
mod tests {
	...
	fn test_strip_line_comment() {
		...
		assert_eq!(strip_line_comment(r#"bla (//) bla//the end"#), r#"bla (//) bla"#);

		// ignores even unescaped URLs
		assert_eq!(strip_line_comment(r#"https://test.com// comment//"#), r#"https://test.com"#);
	}
	...
	fn test_minify() {
		fn test(description:&str, code:&str, expected:&str) {
			// test minify()
			assert_eq!(minify(code, &LINEBREAK_REGEX_RAW), expected, "{}: minify", description);
