...
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprFactory, prepend_stmt, private_ident, quote_ident};
...
pub fn transpile_css_prop(state:Rc<RefCell<State>>) -> impl Pass + VisitMut {
	visit_mut_pass(TranspileCssProp { state, ..Default::default() })
}
...
impl VisitMut for TranspileCssProp {
		...
		for attr in elem.opening.attrs.iter_mut() {
			match &mut *attr {
				JSXAttrOrSpread::JSXAttr(attr) => {
					...
					let import_name = if let Some(ident) =
						self.state.borrow().import_local_name("default", None).map(Ident::from)
					{
						ident
					} else {
						self.import_name.get_or_insert_with(|| private_ident!("_styled")).clone()
					};
					...
					let (styled, inject_after) = if TAG_NAME_REGEX.is_match(&name.sym) {
						(
							(Expr::Call(CallExpr {
								...
								args:vec![
									Lit::Str(Str { span:DUMMY_SP, value:name.sym, raw:None })
										.as_arg(),
								],
								..Default::default()
							})),
						...
						(
							...
							if self.is_top_level_ident(&name) { Some(name) } else { None },
						)
					};
					...
					if let Expr::Object(css_obj) = &mut css {
						...
						css_obj.props =
							css_obj.props.take().into_iter().fold(vec![], |acc, property| {
						...
						tpl.exprs =
							tpl.exprs.take().into_iter().fold(vec![], |mut acc, mut expr| {
								if expr.is_fn_expr()
									|| expr.is_arrow() || is_direct_access(&expr, &|id| {
					...
					match inject_after {
						Some(injector) => {
							...
							self.interleaved_injections.entry(id).or_default().push(stmt);
						},
	...
	fn visit_mut_module(&mut self, n:&mut Module) {
		...
		if let Some(import_name) = self.import_name.take() {
			...
			prepend_stmt(
				...
				ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
					...
					src:Box::new(Str { span:DUMMY_SP, value:"styled-components".into(), raw:None }),
		...
		let mut remaining =
			std::mem::take(&mut self.interleaved_injections).into_iter().collect::<Vec<_>>();
		...
		n.body.extend(self.injected_nodes.take().into_iter().map(ModuleItem::Stmt));
	}
}
...
fn get_name_expr(name:&JSXElementName) -> Box<Expr> {
	...
	match name {
		...
		JSXElementName::JSXMemberExpr(n) => {
			Box::new(Expr::Member(MemberExpr {
				span:DUMMY_SP,
				obj:get_name_expr_jsx_object(&n.obj),
				prop:MemberProp::Ident(n.prop.clone()),
...
fn set_value_of_prop(prop:&mut Prop, value:Box<Expr>) {
	match prop {
		Prop::Shorthand(p) => {
			*prop = Prop::KeyValue(KeyValueProp { key:PropName::Ident(p.clone().into()), value });
		},
...
fn set_key_of_prop(prop:&mut Prop, key:Box<Expr>) {
	...
	*prop = Prop::KeyValue(KeyValueProp {
		key:PropName::Computed(ComputedPropName { span:DUMMY_SP, expr:key }),
		value,
	});
}
