...
use swc_common::{DUMMY_SP, FileName, util::take::Take};
use swc_ecma_ast::*;
...
use crate::{
	Config,
	utils::{State, get_prop_name, prefix_leading_digit},
};
...
static DISPLAY_NAME_REGEX:Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-zA-Z][a-zA-Z0-9]$").unwrap());
...
impl DisplayNameAndId {
	...
	fn add_config(
		...
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) = e {
			if let Expr::Member(MemberExpr { prop: MemberProp::Ident(prop), .. }) = &**callee {
		...
		if let Expr::TaggedTpl(e) = e {
			e.tag = Box::new(Expr::Call(CallExpr {
				callee:e.tag.take().make_member(quote_ident!("withConfig")).as_callee(),
				args:vec![ObjectLit { span:DUMMY_SP, props:with_config_props }.as_arg()],
				..Default::default()
			}));
		...
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = e {
			*callee = Box::new(Expr::Call(CallExpr {
				span:DUMMY_SP,
				callee:callee.take().make_member(quote_ident!("withConfig")).as_callee(),
				args:vec![ObjectLit { span:DUMMY_SP, props:with_config_props }.as_arg()],
				..Default::default()
			}));
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) => {
				(
					...
					self.state.borrow().is_styled(callee)
						&& get_property_as_ident(callee).map(|v| v != "withConfig").unwrap_or(false)
				) || (
					// callee is styled(MyComponent)
					self.state.borrow().is_styled(callee)
						&& !get_callee(callee).map(|callee| callee.is_member()).unwrap_or(false)
				) || (
					...
					self.state.borrow().is_styled(callee)
						&& get_callee(callee)
							.and_then(get_property_as_ident)
							.map(|v| v == "withConfig")
							.unwrap_or(false) && Expr::as_call(callee)
...
fn get_callee(e:&Expr) -> Option<&Expr> {
	match e {
		Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) => Some(callee),
		_ => None,
	}
}

fn get_property_as_ident(e:&Expr) -> Option<&JsWord> {
	if let Expr::Member(MemberExpr { prop: MemberProp::Ident(p), .. }) = e {
		return Some(&p.sym);
	}
		...
		.any(|prop| {
			match prop {
				PropName::Ident(ident) => {
					&*ident.sym == "componentId" || &*ident.sym == "displayName"
				},
				_ => false,
			}
		})
...
where
	F: FnOnce(&mut Expr), {
	if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = e {
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee_callee), .. }) = &mut **callee {
			if let Expr::Member(MemberExpr { prop: MemberProp::Ident(prop), .. }) = &**callee_callee
			{
				if &*prop.sym == "withConfig" {
					return op(callee);
				}
			...
			if let Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(..), .. }) =
				&mut **callee_callee
			{
				if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = &**obj {
					if let Expr::Member(MemberExpr { prop: MemberProp::Ident(prop), .. }) =
						&**callee
					{
						if &*prop.sym == "withConfig" {
							op(obj)
