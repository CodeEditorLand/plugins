	...
	match value {
		...
		Expr::Tpl(tpl) => {
			...
			Some(
				tpl.quasis
					.iter()
					.map(|q| q.cooked.as_ref().map(|v| v.to_string()).unwrap_or("".to_string()))
					.collect::<Vec<String>>()
					.join(""),
			)
		},
		...
		if is_literal_err {
		...
		} else {
			{
				handler.with(|handler| {
					...
					handler.struct_err(&format!("SyntaxError: {}", e.kind)).emit()
				});
			}
		}
	}
...
fn get_call_expr_icu_message_value(message_path:&Option<Expr>, preserve_whitespace:bool) -> String {
	if message_path.is_none() {
		return "".to_string();
	}
...
fn evaluate_jsx_message_descriptor(
	...
	let default_message =
		get_jsx_icu_message_value(&descriptor_path.default_message, options.preserve_whitespace);
	...
	MessageDescriptor { id, default_message:Some(default_message), description }
}
...
fn store_message(
	...
	let source_location = if let Some(location) = location {
		...
		Some(SourceLocation {
			...
			start:Location { line:start.line, col:start.col.to_usize() },
			end:Location { line:end.line, col:end.col.to_usize() },
		})
	} else {
		None
	};
	...
	messages.push(ExtractedMessage {
		id:descriptor.id.as_ref().unwrap_or(&"".to_string()).to_string(),
		default_message:descriptor.default_message.as_ref().expect("Should be available").clone(),
...
impl<C:Clone + Comments, S:SourceMapper> FormatJSVisitor<C, S> {
	fn new(
		...
		plugin_options.additional_function_names.iter().for_each(|name| {
		...
		plugin_options.additional_component_names.iter().for_each(|name| {
		...
		if let Some(message_obj) = &mut *message_descriptor {
			...
			if let Expr::Object(obj) = *message_obj {
				...
				let source_location = if self.options.extract_source_location {
					Some((self.source_map.lookup_char_pos(lo), self.source_map.lookup_char_pos(hi)))
				} else {
					None
				};

				store_message(&mut self.messages, &descriptor, &self.filename, source_location);
...
impl<C:Clone + Comments, S:SourceMapper> VisitMut for FormatJSVisitor<C, S> {
	...
	fn visit_mut_jsx_opening_element(&mut self, jsx_opening_elem:&mut JSXOpeningElement) {
		...
		let id_attr = jsx_opening_elem.attrs.iter().find(|attr| {
			match attr {
		...
		if descriptor.id.is_some() {
			if let Some(id_attr) = id_attr {
				if let JSXAttrOrSpread::JSXAttr(attr) = id_attr {
					attr.to_owned().value =
						Some(JSXAttrValue::Lit(Lit::Str(Str::from(descriptor.id.unwrap()))));
				}
			} else if first_attr {
				jsx_opening_elem.attrs.insert(
					...
					JSXAttrOrSpread::JSXAttr(JSXAttr {
						...
						value:Some(JSXAttrValue::Lit(Lit::Str(Str::from(descriptor.id.unwrap())))),
					}),
				)
			}
		}
...
fn json_value_to_expr(json_value:&serde_json::Value) -> Box<Expr> {
	Box::new(match json_value {
		...
		serde_json::Value::Bool(v) => Expr::Lit(Lit::Bool(Bool { span:DUMMY_SP, value:*v })),
		serde_json::Value::Number(v) => {
			Expr::Lit(Lit::Num(Number { span:DUMMY_SP, raw:None, value:v.as_f64().unwrap() }))
		},
		serde_json::Value::String(v) => {
			Expr::Lit(Lit::Str(Str { span:DUMMY_SP, raw:None, value:v.as_str().into() }))
		},
		serde_json::Value::Array(v) => {
			Expr::Array(ArrayLit {
				...
				elems:v
					.iter()
					.map(|elem| Some(ExprOrSpread { spread:None, expr:json_value_to_expr(elem) }))
					.collect(),
			})
		},
