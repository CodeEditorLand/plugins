...
impl From<&str> for Transform {
	fn from(s:&str) -> Self { Transform::String(s.to_string()) }
}
impl From<Vec<(String, String)>> for Transform {
	fn from(v:Vec<(String, String)>) -> Self { Transform::Vec(v) }
}
		...
		for (regex, config) in &self.packages {
			...
			if let Some(group) = group {
				let group = group
					...
					.map(|x| x.map(|x| x.as_str()).unwrap_or_default())
					.collect::<Vec<&str>>();
				return Some(Rewriter { renderer:&self.renderer, key:name, config, group });
			}
		}
		None
	}
...
pub fn modularize_imports(config:Config) -> impl Fold {
	let mut folder = FoldImports { renderer:handlebars::Handlebars::new(), packages:vec![] };
