...
impl ImportMap {
		...
		match e {
			Expr::Ident(i) => {
				if let Some((i_src, i_sym, i_span)) = self.imports.get(&i.to_id()) {
					if i_src == module && i_sym == orig_name { Some(*i_span) } else { None }
				} else {
					None
				}
			},
			...
			Expr::Member(MemberExpr {
				...
				if let Some((obj_src, obj_span)) = self.namespace_imports.get(&obj.to_id()) {
					if obj_src == module && prop.sym == *orig_name { Some(*obj_span) } else { None }
				} else {
					None
				}
			},
	...
	pub fn is_in_import_items(&self, e:&Expr, import_items:&[ImportItem]) -> Option<Span> {
		import_items.iter().find_map(|item| self.is_import(e, &item.module, &item.name))
	}
