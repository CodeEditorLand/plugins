use swc_common::{Spanned, comments::Comments, errors::HANDLER};
use swc_ecma_ast::{
	Expr,
	KeyValueProp,
	ObjectLit,
	...
	C: Comments,
{
	...
	pub(super) fn transform_flag(&mut self, v:&mut VarDeclarator) -> Option<!> {
		...
		let import_of_flag_callee =
			self.imports.is_in_import_items(callee, &self.config.flag.import_sources)?;
		...
		if call_expr.args.is_empty() {
			call_expr
				.args
				.push(ObjectLit { props:vec![prop], ..Default::default() }.as_arg());
		} else if let Some(obj) = find_object(&mut call_expr.args[0].expr) {
			if obj.props.iter().filter_map(|p| p.as_prop()).any(|p| {
				match &**p {
	...
	match arg {
		...
		Expr::Call(call) => {
			if call.args.is_empty() {
				call.args.push(ObjectLit { ..Default::default() }.as_arg());
			}
