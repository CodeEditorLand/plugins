...
use swc_common::{BytePos, DUMMY_SP, SourceMapperDyn, comments::Comments, util::take::Take};
use swc_ecma_ast::{
	ArrayLit,
	CallExpr,
	Callee,
...
static EMOTION_OFFICIAL_LIBRARIES:Lazy<Vec<EmotionModuleConfig>> = Lazy::new(|| {
	vec![
		EmotionModuleConfig {
			...
			exported_names:vec![ExportItem { name:"css".to_owned(), kind:ExprKind::Css }],
		...
		EmotionModuleConfig {
			...
			exported_names:vec![
				ExportItem { name:"css".to_owned(), kind:ExprKind::Css },
				ExportItem { name:"keyframes".to_owned(), kind:ExprKind::Css },
				ExportItem { name:"Global".to_owned(), kind:ExprKind::GlobalJSX },
			],
			..Default::default()
		},
		EmotionModuleConfig {
...
static INVALID_CSS_CLASS_NAME_CHARACTERS:Lazy<Regex> =
...
static MULTI_LINE_COMMENT:Lazy<Regex> =
	Lazy::new(|| RegexBuilder::new(r"(?s)/\*.*?\*/").multi_line(true).build().unwrap());
...
pub fn emotion<C:Comments>(
	...
	fold_pass(EmotionTransformer::new(emotion_options, path, src_file_hash, cm, comments))
}
...
impl<C:Comments> EmotionTransformer<C> {
	...
	fn sanitize_label_part<'t>(&self, label_part:&'t str) -> String {
		...
		INVALID_CSS_CLASS_NAME_CHARACTERS.replace_all(&without_spaces, "-").to_string()
	}
	...
	fn create_label(&self, with_prefix:bool) -> String {
		...
		let mut label = format!(
			"{}{}",
			prefix,
			self.options.label_format.clone().unwrap_or_else(|| "[local]".to_owned())
		);
	...
	fn rewrite_styles_attr(&mut self, attrs:&mut [JSXAttrOrSpread], pos:BytePos) {
		if let Some(attr_value) = attrs.iter_mut().find_map(|attr| {
			if let JSXAttrOrSpread::JSXAttr(JSXAttr {
				name: JSXAttrName::Ident(i), value, ..
			}) = attr
			{
				if i.as_ref() == "styles" {
					return value.as_mut();
					...
					if let Expr::Ident(i) = m.obj.as_ref() {
						if let Some(package) = self.import_packages.get(&i.to_id()) {
							...
							if let PackageMeta::Namespace(c) = package {
								if c.exported_names.iter().any(|n| match_css_export(n, &m.prop)) {
									self.comments.add_pure_comment(expr.span.lo());
...
fn minify_css_string(input:&str, is_first_item:bool, is_last_item:bool) -> Cow<str> {
	match MULTI_LINE_COMMENT.replace_all(input, "$s") {
		...
		Cow::Owned(owned) => {
			remove_space_and_comments(&owned, is_first_item, is_last_item)
				.into_owned()
				.into()
		},
	}
...
fn remove_space_and_comments(input:&str, is_first_item:bool, is_last_item:bool) -> Cow<str> {
	match INVALID_SINGLE_LINE_COMMENT.replace_all(input, "$s") {
		...
		Cow::Owned(owned) => {
			remove_space_around_colon(&owned, is_first_item, is_last_item)
				.into_owned()
				.into()
		},
	}
...
fn remove_space_around_colon(input:&str, is_first_item:bool, is_last_item:bool) -> Cow<str> {
	...
	SPACE_AROUND_COLON.replace_all(
		input
			.trim_start_matches(if is_first_item { pattern_trim_spaces } else { pattern })
			.trim_end_matches(if is_last_item { pattern_trim_spaces } else { pattern }),
		"$s",
	)
}
...
mod test_emotion {
	...
	fn issue_258_should_preserve_url_starting_with_two_slashes_1() {
		assert_eq!(
			minify_css_string("background-image: url('//domain.com/image.png');", true, true),
