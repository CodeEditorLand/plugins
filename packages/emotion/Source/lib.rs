...
use swc_common::{SourceMapper, Spanned, plugin::metadata::TransformPluginMetadataContextKind};
...
pub fn process_transform(program:Program, data:TransformPluginProgramMetadata) -> Program {
	...
	program.apply(swc_emotion::emotion(config, path, hash, source_map, data.comments))
}
