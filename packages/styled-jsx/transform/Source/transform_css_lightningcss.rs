...
use parcel_selectors::{SelectorImpl, parser::SelectorIter};
use preset_env_base::{Versions, version::Version};
use swc_common::{
	BytePos,
	DUMMY_SP,
	Loc,
...
pub fn transform_css(
	...
	let mut ss = match result {
		...
		Err(err) => {
			HANDLER.with(|handler| {
				report(&cm, style_info.css_span, &mut file_lines_cache, &err, Level::Error);
	...
	let targets = Targets { browsers:Some(convert_browsers(browsers)), ..Default::default() };

	// Apply auto prefixer
	ss.minify(MinifyOptions {
		targets:Targets { exclude:Features::CustomMediaQueries, ..targets },
		..Default::default()
	})
...
fn count_occurrences(s:impl AsRef<str>, c:char) -> usize { s.as_ref().split(c).count() - 1 }
...
pub(crate) fn strip_comments(s:&str) -> String { s.lines().map(strip_line_comment).collect() }
...
impl<'i> Visitor<'i> for CssNamespace {
	...
	fn visit_types(&self) -> VisitTypes { visit_types!(SELECTORS) }
	...
	fn visit_selector(&mut self, selector:&mut Selector<'i>) -> Result<(), Self::Error> {
		...
		let _tracing =
			tracing::span!(tracing::Level::ERROR, "visit_selector", len = selector.len()).entered();
		...
		loop {
			...
			trace!("Selector length after transform: {}", iter.selector_length());
		...
		let new:Vec<_> =
			RemoveWhitespace { iter:new_selectors.into_iter().rev().flatten(), prev:None }
				.collect();
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self.prev.take() {
			...
			_ => {
				...
				if self.prev.is_some() { self.next() } else { None }
			},
		}
	}
}
	...
	where
		...
		match pseudo_index {
			...
			Some(insert_index) => {
				...
				if !self.is_global {
					result.insert(insert_index, Component::Class(Ident::from(subclass_selector)));
				}
			},
		}
