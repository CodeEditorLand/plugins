...
use swc_common::{DUMMY_SP, FileName, SourceMap, Span, collections::AHashSet, errors::HANDLER};
use swc_ecma_ast::*;
...
use crate::{
	...
	utils::{
		...
		and,
		compute_class_names,
		get_usable_import_specifier,
		hash_string,
		ident,
		is_capitalized,
		make_external_styled_jsx_el,
...
pub fn styled_jsx(
	...
	let file_name = match file_name {
		FileName::Real(real_file_name) => {
			real_file_name.to_str().map(|real_file_name| real_file_name.to_string())
		},
		_ => None,
	};
...
impl Fold for StyledJSXTransformer<'_> {
		...
		if let JSXElementName::Ident(Ident { sym, ctxt, .. }) = &el.name {
					...
					|| self.nearest_scope_bindings.contains(&(sym.clone(), *ctxt)))
			{
				...
				let new_class_name = match (existing_class_name, &self.class_name) {
					(Some(existing_class_name), Some(class_name)) => {
						Some(add(
	...
	fn fold_import_decl(&mut self, decl:ImportDecl) -> ImportDecl {
		let ImportDecl { ref src, ref specifiers, .. } = decl;
		...
		match expr {
			Expr::TaggedTpl(tagged_tpl) => {
				match &*tagged_tpl.tag {
					...
					Expr::Member(MemberExpr { obj: boxed_ident, .. }) => {
						if let Expr::Ident(identifier) = &**boxed_ident {
							if self.external_bindings.contains(&identifier.to_id()) {
								match self
									.process_tagged_template_expr(&tagged_tpl, &identifier.sym)
								{
									Ok(expr) => expr,
									Err(_) => Expr::TaggedTpl(tagged_tpl),
								}
	...
	fn fold_var_declarator(&mut self, declarator:VarDeclarator) -> VarDeclarator {
		...
		if let Some(external_hash) = &self.external_hash.take() {
			if let Pat::Ident(BindingIdent { id: Ident { ctxt, sym, .. }, .. }) = &declarator.name {
	...
	fn fold_export_default_expr(&mut self, default_expr:ExportDefaultExpr) -> ExportDefaultExpr {
		...
		if let Some(external_hash) = &self.external_hash.take() {
			...
			return ExportDefaultExpr { expr:Box::new(Expr::Ident(default_ident)), span:DUMMY_SP };
		}
...
impl StyledJSXTransformer<'_> {
	...
	fn get_jsx_style(&mut self, style_expr:StyleExpr, is_global_jsx_element:bool) -> JSXStyle {
		...
		match style_expr {
			...
			StyleExpr::Tpl(Tpl { exprs, quasis, span }, expr) => {
				if exprs.is_empty() {
	...
	fn replace_jsx_style(&mut self, el:&JSXElement) -> Result<JSXElement, Error> {
		...
		match &style_info {
			...
			JSXStyle::External(style) => {
				Ok(make_external_styled_jsx_el(style, self.style_import_name.as_ref().unwrap()))
			},
		}
	}
	...
	fn process_tagged_template_expr(
		...
		let tag = match &*tagged_tpl.tag {
			...
			Expr::Member(MemberExpr { prop: MemberProp::Ident(IdentName { sym, .. }), .. }) => {
				sym.to_string()
			},
		...
		Ok(Expr::New(NewExpr {
			callee:Box::new(Expr::Ident(Ident { sym:"String".into(), ..Default::default() })),
			args:Some(vec![ExprOrSpread { expr:Box::new(css), spread:None }]),
			..Default::default()
		}))
	}
...
fn get_existing_class_name(el:&JSXOpeningElement) -> (Option<Expr>, Option<usize>, Option<usize>) {
	...
	(existing_class_name_expr, existing_index, existing_spread_index)
}
...
fn add_hash_statement((id, hash):(Id, String)) -> Stmt {
	Stmt::Expr(ExprStmt {
		expr:Box::new(Expr::Assign(AssignExpr {
			left:MemberExpr {
				obj:Box::new(Expr::Ident(Ident { sym:id.0, ctxt:id.1, ..Default::default() })),
				prop:MemberProp::Ident(IdentName { sym:"__hash".into(), span:DUMMY_SP }),
				span:DUMMY_SP,
			}
			.into(),
