...
fn tpl_element(value:&str) -> TplElement {
	TplElement { raw:value.into(), cooked:None, span:DUMMY_SP, tail:false }
}
...
pub fn compute_class_names(
	...
	if !external_styles.is_empty() {
		...
		external_jsx_id = Some(Expr::Tpl(Tpl {
			quasis,
			exprs:external_styles.iter().map(|external| Box::new(external.clone())).collect(),
			span:DUMMY_SP,
		}));
	}
		...
		_ => {
			Some(Expr::Call(CallExpr {
				callee:Callee::Expr(Box::new(Expr::Member(MemberExpr {
					...
					prop:MemberProp::Ident(IdentName { sym:"dynamic".into(), span:DUMMY_SP }),
					span:DUMMY_SP,
				}))),
						...
						elems:dynamic_styles
							...
							.map(|style_info| {
								let hash_input = match &static_class_name {
									Some(class_name) => {
										format!("{}{}", style_info.hash, class_name)
									},
	...
	let class_name_expr = match (static_class_name.clone(), dynamic_class_name, external_jsx_id) {
		(Some(static_class_name), Some(dynamic_class_name), Some(external_jsx_id)) => {
			Some(add(
				add(external_jsx_id, string_literal_expr(&format!(" {} ", static_class_name))),
				dynamic_class_name,
			))
		},
		(Some(static_class_name), Some(dynamic_class_name), None) => {
			Some(add(string_literal_expr(&format!("{} ", static_class_name)), dynamic_class_name))
		},
		(Some(static_class_name), None, Some(external_jsx_id)) => {
			Some(add(string_literal_expr(&format!("{} ", static_class_name)), external_jsx_id))
		},
		(None, Some(dynamic_class_name), Some(external_jsx_id)) => {
			Some(add(add(external_jsx_id, string_literal_expr(" ")), dynamic_class_name))
		},
...
pub fn make_external_styled_jsx_el(style:&ExternalStyle, style_import_name:&str) -> JSXElement {
	let attrs = vec![JSXAttrOrSpread::JSXAttr(JSXAttr {
		name:JSXAttrName::Ident(IdentName { sym:"id".into(), ..Default::default() }),
	...
	let opening = JSXOpeningElement {
		name:JSXElementName::Ident(Ident { sym:style_import_name.into(), ..Default::default() }),
		attrs,
		span:DUMMY_SP,
		self_closing:false,
		type_args:None,
	...
	JSXElement { opening, closing, children, span:DUMMY_SP }
}
...
pub fn make_local_styled_jsx_el(
	...
	let mut attrs = vec![JSXAttrOrSpread::JSXAttr(JSXAttr {
		name:JSXAttrName::Ident(IdentName { sym:"id".into(), ..Default::default() }),
		value:Some(JSXAttrValue::JSXExprContainer(JSXExprContainer {
			expr:JSXExpr::Expr(Box::new(string_literal_expr(hash_string(&hash_input).as_str()))),
			span:DUMMY_SP,
		})),
		span:DUMMY_SP,
	})];
	...
	if style_info.is_dynamic {
		attrs.push(JSXAttrOrSpread::JSXAttr(JSXAttr {
			name:JSXAttrName::Ident(IdentName { sym:"dynamic".into(), span:DUMMY_SP }),
			value:Some(JSXAttrValue::JSXExprContainer(JSXExprContainer {
				expr:JSXExpr::Expr(Box::new(Expr::Array(ArrayLit {
					elems:style_info
						...
						.iter()
						.map(|expression| {
							Some(ExprOrSpread { expr:expression.clone(), spread:None })
						})
						.collect(),
					span:DUMMY_SP,
				}))),
...
pub fn styled_jsx_import_decl(style_import_name:&str) -> ModuleItem {
	ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
		...
		specifiers:vec![ImportSpecifier::Default(ImportDefaultSpecifier {
			local:Ident { sym:style_import_name.into(), ..Default::default() },
			span:DUMMY_SP,
		})],
		src:Box::new(Str { span:DUMMY_SP, value:"styled-jsx/style".into(), raw:None }),
...
pub fn string_literal_expr(s:&str) -> Expr { s.replace("\\`", "`").into() }

pub fn ident(s:&str) -> IdentName { IdentName { sym:s.into(), ..Default::default() } }

pub fn is_capitalized(word:&str) -> bool { word.chars().next().unwrap().is_uppercase() }

pub fn add(left:Expr, right:Expr) -> Expr { binary_expr(BinaryOp::Add, left, right) }

pub fn and(left:Expr, right:Expr) -> Expr { binary_expr(BinaryOp::LogicalAnd, left, right) }

pub fn or(left:Expr, right:Expr) -> Expr { binary_expr(BinaryOp::LogicalOr, left, right) }

pub fn not_eq(left:Expr, right:Expr) -> Expr { binary_expr(BinaryOp::NotEq, left, right) }

pub fn binary_expr(op:BinaryOp, left:Expr, right:Expr) -> Expr {
	Expr::Bin(BinExpr { op, left:Box::new(left), right:Box::new(right), span:DUMMY_SP })
}
