...
pub fn transform_css(
	...
	let config =
		ParserConfig { allow_wrong_line_comments:true, css_modules:false, ..Default::default() };
		...
		for (i, selector) in node.subclass_selectors.iter().enumerate() {
			...
			if &name.value == "global" {
				...
				let mut tokens = {
					...
					Tokens { span:Span::new(lo, hi), tokens }
				};
...
fn get_front_selector_tokens(selector_tokens:&Tokens) -> Vec<TokenAndSpan> {
	...
	vec![
		TokenAndSpan {
			span:Span { lo:BytePos(start_pos), hi:BytePos(start_pos + 1) },
			token:Token::Ident { raw:"a".into(), value:"a".into() },
		},
		TokenAndSpan {
			span:Span { lo:BytePos(start_pos + 1), hi:BytePos(start_pos + 2) },
