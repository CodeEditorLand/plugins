...
use swc_common::{FileName, plugin::metadata::TransformPluginMetadataContextKind};
use swc_core::{
	ecma::ast::Program,
	plugin::{plugin_transform, proxies::TransformPluginProgramMetadata},
};
use swc_relay::{Config, OutputFileExtension, ProjectConfig, RelayLanguageConfig, relay};
...
fn relay_plugin_transform(program:Program, metadata:TransformPluginProgramMetadata) -> Program {
	...
	let relay = relay(config.into(), filename, root_dir, None, Some(metadata.unresolved_mark));
