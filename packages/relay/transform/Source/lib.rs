...
use swc_common::{DUMMY_SP, FileName, Mark, SyntaxContext};
use swc_ecma_ast::*;
...
fn build_require_expr_from_path(path:&str, mark:Option<Mark>) -> Expr {
	Expr::Call(CallExpr {
		span:Default::default(),
		callee:quote_ident!(
			mark.map(|m| SyntaxContext::empty().apply_mark(m)).unwrap_or_default(),
			"require"
		)
		.as_callee(),
		args:vec![
			Lit::Str(Str { span:Default::default(), value:JsWord::from(path), raw:None }).as_arg(),
		],
		..Default::default()
	})
}
...
impl Fold for Relay {
	...
	fn fold_module_items(&mut self, items:Vec<ModuleItem>) -> Vec<ModuleItem> {
		let mut items =
			items.into_iter().map(|item| item.fold_children_with(self)).collect::<Vec<_>>();

		prepend_stmts(&mut items, self.imports.iter().map(|import| import.as_module_item()));
...
impl Relay {
	fn path_for_artifact(
			...
			.map_or(false, |pages_dir| real_file_name.starts_with(pages_dir))
		{
			...
			Ok(real_file_name.parent().unwrap().join("./__generated__").join(filename))
		}
	}
	...
	fn build_call_expr_from_tpl(&mut self, tpl:&TaggedTpl) -> Option<Expr> {
		...
		match operation_name {
			None => None,
			Some(operation_name) => {
				match self.build_require_path(&operation_name) {
					Ok(final_path) => {
						...
						if self.config.eager_es_modules {
							...
							Some(build_require_expr_from_path(&final_path, self.unresolved_mark))
						}
					},

					Err(_err) => {
						// let base_error = "Could not transform GraphQL template to a Relay
						// import."; let error_message = match err {
...
pub fn relay(
	...
	fold_pass(Relay { root_dir, file_name, config, pages_dir, imports:vec![], unresolved_mark })
}
