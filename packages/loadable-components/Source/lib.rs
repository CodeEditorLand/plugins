	...
	C: Comments,
{
	...
	fn get_chunk_name_content(&self, import_arg:&Expr) -> Option<String> {
		...
		self.comments.with_leading(import_arg.span_lo(), |comments| {
			comments
				.iter()
...
impl Signature {
	fn is_default_specifier(&self) -> bool { self.name == *"default" }

	pub fn default_lazy() -> Self {
		Signature { name:"lazy".into(), from:"@loadable/component".into() }
	}
}
