...
pub enum ErrorKind {
	...
	/// Expecting an offset value in `plural` or `selectordinal` argument (e.g
	/// `{foo, plural, offset}`)
	ExpectPluralArgumentOffsetValue = 13,
	/// Offset value in `plural` or `selectordinal` is invalid (e.g. `{foo,
	/// plural, offset: x}`)
	InvalidPluralArgumentOffsetValue = 14,

	/// Expecting a selector in `select` argument (e.g `{foo, select}`)
	ExpectSelectArgumentSelector = 15,
	/// Expecting a selector in `plural` or `selectordinal` argument (e.g `{foo,
	/// plural}`)
	ExpectPluralArgumentSelector = 16,

	/// Expecting a message fragment after the `select` selector (e.g. `{foo,
	/// select, apple}`)
	...
	/// Selector in `plural` or `selectordinal` is malformed (e.g. `{foo,
	/// plural, =x {#}}`)
...
///
/// A position encodes one half of a span, and include the code unit offset,
...
pub enum AstElement<'s> {
	/// Raw text
	Literal { value:String, span:Option<Span> },
	/// Variable w/o any format, e.g `var` in `this is a {var}`
	Argument { value:String, span:Option<Span> },
	/// Variable w/ number format
	Number { value:String, span:Option<Span>, style:Option<NumberArgStyle<'s>> },
	/// Variable w/ date format
	Date { value:String, span:Option<Span>, style:Option<DateTimeArgStyle<'s>> },
	/// Variable w/ time format
	Time { value:String, span:Option<Span>, style:Option<DateTimeArgStyle<'s>> },
	/// Variable w/ select format
	Select { value:String, span:Option<Span>, options:PluralOrSelectOptions<'s> },
	...
	Pound(Span),
	/// XML-like tag
	Tag { value:&'s str, span:Option<Span>, children:Box<Ast<'s>> },
}
