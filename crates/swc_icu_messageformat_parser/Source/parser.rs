use std::{cell::Cell, cmp, collections::HashSet, result};
...
use crate::{
	ast::{self, *},
	intl::{
		date_time_format_options::JsIntlDateTimeFormatOptions,
		number_format_options::JsIntlNumberFormatOptions,
		options::{
			CompactDisplay,
			DateTimeDisplayFormat,
			DateTimeMonthDisplayFormat,
			HourCycle,
			Notation,
...
pub static CONCISE_INTEGER_WIDTH_REGEX:Lazy<Regexp> = Lazy::new(|| Regexp::new(r"^(0+)$").unwrap());
...
pub struct ParserOptions {
	...
	/// into Intl.NumberFormatOptions and Intl.DateTimeFormatOptions,
	/// respectively
	#[serde(default)]
...
fn is_alpha(ch:Option<char>) -> bool {
	if let Some(ch) = ch { ch.is_ascii_alphabetic() } else { false }
}
...
fn get_default_hour_symbol_from_locale(locale:&str) -> char {
	...
	// There's no built in Intl.Locale, manually read through extensions for the
...
fn parse_significant_precision(ret:&mut JsIntlNumberFormatOptions, value:&str) {
	...
	if let Some(cap) = cap {
		...
		let is_g2_non_str =
			g2.is_none() || g2.map(|g| g.as_str().parse::<u32>().is_ok()).unwrap_or(false);
...
impl<'s> Parser<'s> {
	...
	/// # Arguments
	///
	/// * `nesting_level` - The nesting level of the message. This can be
	///   positive if the message is nested inside the plural or select
	///   argument's selector clause.
	/// * `parent_arg_type` - If nested, this is the parent plural or selector's
	///   argument type. Otherwise this should just be an empty string.
	/// * `expecting_close_tag` - If true, this message is directly or
	///   indirectly nested inside between a pair of opening and closing tags.
	///   The nested message will not parse beyond the closing tag boundary.
	fn parse_message(
		&self,
		nesting_level:usize,
		parent_arg_type:&str,
	...
	fn position(&self) -> Position { self.position.get() }

	/// A tag name must start with an ASCII lower case letter. The grammar is
	/// based on the [custom element name][] except that a dash is NOT
	/// always mandatory and uppercase letters are accepted:
	///
	/// ```ignore
	...
	fn parse_literal(&self, nesting_level:usize, parent_arg_type:&str) -> Result<AstElement> {
		...
		Ok(AstElement::Literal {
			span:if self.options.capture_location { Some(span) } else { None },
			value,
		})
	}

	/// Starting with ICU 4.8, an ASCII apostrophe only starts quoted text if it
	/// immediately precedes a character that requires quoting (that is,
	/// "only where needed"), and works the same in nested messages as on
	/// the top level of the pattern. The new behavior is otherwise compatible.
	fn try_parse_quote(&self, parent_arg_type:&str) -> Option<String> {
		if self.is_eof() || self.char() != '\'' {
			return None;
		}

		// Parse escaped char following the apostrophe, or early return if there is no
		// escaped char. Check if is valid escaped character
		match self.peek() {
			Some('{') | Some('<') | Some('>') | Some('}') => (),
	...
	fn parse_argument(&self, nesting_level:usize, expecting_close_tag:bool) -> Result<AstElement> {
	...
	fn parse_argument_options(
		...
		match arg_type {
			"" => {
				// Expecting a style string number, date, time, plural, selectordinal, or
				// select.
				Err(self.error(
	...
	/// * `nesting_level` - the current nesting level of messages. This can be
	///   positive when parsing message fragment in select or plural argument
	///   options.
	/// * `parent_arg_type` - the parent argument's type.
	/// * `parsed_first_identifier` - if provided, this is the first
	///   identifier-like selector of the argument. It is a by-product of a
	...
	/// Advance the parser until the end of the identifier, if it is currently
	/// on an identifier character. Return an empty string otherwise.
	#[cfg(feature = "utf16")]
	fn parse_identifier_if_possible(&self) -> (&Utf16Str, Span) {
		let span = self.parse_identifier_if_possible_inner();
		(&self.message_utf16[span.start.offset..span.end.offset], span)
	}
	...
	fn error(&self, kind:ErrorKind, span:Span) -> ast::Error {
		ast::Error {
			...
			location:if self.options.capture_location { Some(span) } else { None },
		}
	}

	fn offset(&self) -> usize { self.position().offset }
	...
	/// advance the parser through all whitespace to the next non-whitespace
	/// byte.
	fn bump_space(&self) {
...
fn parse_number_skeleton_from_string(
	...
	Ok(NumberSkeleton {
		...
		location:if should_capture_location { Some(span) } else { None },
		parsed_options,
	})
}
